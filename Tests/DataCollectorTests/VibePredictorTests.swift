...
struct VibePredictorTests {
    ...
    func modelLoads() async {
        let _ = VibePredictor()
        // Predictor initializes successfully (non-optional)
    }
    ...
    func sleepPrediction() async {
        ...
        // Test sleep prediction at 3 AM UTC (1704078000)
        let timestamp = Date(timeIntervalSince1970: 1_704_078_000)  // 3 AM UTC
        let result = await predictor.predict(
            timestamp: timestamp,
            distance: 0.0,
            activity: .stationary,
            startTime: timestamp.addingTimeInterval(-3600),  // 1 hour buffer
            duration: 3600
        )
